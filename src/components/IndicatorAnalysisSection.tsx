...
import { ArrowLeft, Save, AlertTriangle, CheckCircle, Download, Loader2 } from 'lucide-react';
...
export const HistoricalDataManagementPage: React.FC<HistoricalDataManagementPageProps> = ({
  ...
  return (
    <div className="p-4 sm:p-6 bg-base-200 shadow-xl rounded-xl">
      ...
      <div className="space-y-8">
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      ...
                      <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-3 gap-y-2`}>
                        {yearEntry.periodicData.map((pd, pIdx) => (
                          <div key={pd.periodLabel} className={`flex items-center space-x-2`}>
                            <span className="text-xs text-gray-600 font-medium w-20 text-right">{pd.periodLabel}:</span>
        ...
        <div className="mt-8 flex flex-col items-center">
          ...
          >
            ...
            {isLoading ? 'Gerando...' : `Gerar e Baixar Arquivo de Dados`}
          </button>
          {saveStatus && (
             <div className={`mt-3 p-2.5 rounded-md text-sm w-full lg:w-2/3 text-center flex items-start ${
                saveStatus.type === 'success' ? 'bg-green-50 text-green-700' :
                saveStatus.type === 'info' ? 'bg-blue-50 text-blue-700' :
                'bg-red-50 text-red-700'
            }`}>
              {saveStatus.type === 'success' && <CheckCircle size={20} className="mr-2 flex-shrink-0"/>}
              {saveStatus.type === 'info' && <AlertTriangle size={20} className="mr-2 flex-shrink-0"/>}
              {saveStatus.type === 'error' && <AlertTriangle size={20} className="mr-2 flex-shrink-0"/>}
