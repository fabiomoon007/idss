...
const getDimensionIcon = (dimId: IDSSDimensionName) => {
  ...
  switch (dimId) {
    case IDSSDimensionName.IDQS: return <CheckSquare {...commonProps} color="#00995d" />; // secondary
    case IDSSDimensionName.IDGA: return <Zap {...commonProps} color="#f47920" />; // accent
      ...
      {dimensions.map((dim: Dimension) => (
        <div key={dim.id} className="bg-base-200 shadow-xl rounded-xl p-6 flex flex-col">
          ...
          <button
            ...
            className="w-full bg-secondary hover:bg-secondary-focus text-white font-semibold py-2 px-3 rounded-md shadow-md transition duration-150 ease-in-out mb-3 text-sm flex items-center justify-center disabled:opacity-60"
