...
import { Indicator, IDSSDimensionName, OperatorSize, CalcularNotaFinalFunction, Periodicity, IDSSIndicatorWeightLevel, PeriodicEntry } from './types.js';
...
const averageConsolidation: (periodicValues: (number | null)[]) => number | null = (periodicValues: (number | null)[]) => {
  const validValues = periodicValues.filter((v): v is number => v !== null);
...
const calcularNotaFinalPartoCesareo: CalcularNotaFinalFunction = (currentRate: number | null, _auxValue?: number | null, operatorSize?: OperatorSize, params?: any) => {
...
const calcularNotaFinalTaxaConsultaPrenatal: CalcularNotaFinalFunction = (value: number | null) => { 
...
const calcularNotaFinalFraturaFemur: CalcularNotaFinalFunction = (value: number | null, _aux?: number | null, operatorSize?: OperatorSize, params?: any) => { 
  if (value === null || !operatorSize || !params || !params[operatorSize]?.meta || !params[operatorSize]?.limiteSuperior) return null;
  const { meta, limiteSuperior } = params[operatorSize];
  return interpolateNotaFinal(value, limiteSuperior, meta, true);
};

const calcularNotaFinalTaxaPediatria: CalcularNotaFinalFunction = (value: number | null) => { 
    if (value === null) return null;
...
const calcularNotaFinalCitopatologia: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
...
const calcularNotaFinalHemoglobinaGlicada: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
  if (value >= 2) return 1;
...
const calcularNotaFinalTaxaConsultasIdososGeneralista: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
...
const calcularNotaFinalPontuacaoBase: CalcularNotaFinalFunction = (value: number | null) => (value === 1 ? 1 : 0);

const calcularNotaFinalHemodialise: CalcularNotaFinalFunction = (value: number | null, auxValue?: number | null) => { 
  if (value === null) return null;
  let notaFinal = 0;
...
const calcularNotaFinalTaxaConsultasMedicasGeneralistaIdosos: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
  if (value >= 2) return 1;
...
const calcularNotaFinalIndiceDispersaoUrgenciaEmergencia: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
...
const calcularNotaFinalFrequenciaRedeHospitalQualidade: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
...
const calcularNotaFinalFrequenciaRedeSADTQualidade: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
...
const calcularNotaFinalICR: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
  if (value >= 3.5) return 1; 
  if (value >= 2.0 && value < 3.5) return 0.975; 
  if (value >= 1.3 && value < 2.0) return 0.95;  
  if (value >= 1.0 && value < 1.3) return 0.90;  
  return 0; 
};

const calcularNotaFinalTaxaResolutividadeNIP: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
...
const calcularNotaFinalIGRAnual: CalcularNotaFinalFunction = (value: number | null, _aux?: number | null, opSize?: OperatorSize, params?: any) => { 
  if (value === null || !opSize || !params || !params[opSize]) return null;
  const { metaIGR, piorIGR } = params[opSize]; 
  if (metaIGR === undefined || piorIGR === undefined) return null;
  return interpolateNotaFinal(value, piorIGR, metaIGR, true);
};

const calcularNotaFinalProporcaoNTRPsAtipicos: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
...
const calcularNotaFinalReajusteMedioPonderado: CalcularNotaFinalFunction = (mediaReajusteOperadora: number | null, cvReajustes?: number | null, _opSize?: OperatorSize, params?: any) => { 
...
const calcularNotaFinalQualidadeCadastralSIB: CalcularNotaFinalFunction = (value: number | null, percDependentesMenoresValidos?: number | null) => { 
  if (value === null) return null;
  let notaFinalBase = 0;
...
const calcularNotaFinalTaxaUtilizacaoSUS: CalcularNotaFinalFunction = (value: number | null, _aux?: number | null, opSize?: OperatorSize, params?: any) => { 
  if (value === null || !opSize || !params || !params[opSize]?.P80 || !params[opSize]?.P97_5) return null;
  const { P80, P97_5 } = params[opSize];
  if (value <= P80) return 1;
  if (value >= P97_5) return 0;
  return interpolateNotaFinal(value, P97_5, P80, true);
};

const calcularNotaFinalRazaoCompletudeTISS: CalcularNotaFinalFunction = (value: number | null) => { 
  if (value === null) return null;
  if (value >= 1) return 1;
...
const calcularNotaFinalProporcaoGlosas: CalcularNotaFinalFunction = (value: number | null, _aux?: number | null, opSize?: OperatorSize, params?: any) => { 
  if (value === null || !opSize || !params || !params[opSize]?.P15 || !params[opSize]?.P85) return null;
  const { P15, P85 } = params[opSize];
  if (value <= P15) return 1;
  if (value > P85) return 0;
...
export const INITIAL_INDICATORS: Indicator[] = [
  ...
  },
  {
    ...
    calcularNotaFinalFn: calcularNotaFinalPontuacaoBase, valueLabel: "Participou (1=Sim, 0=Não)", valueConsolidationFn: (v: (number | null)[]) => v[0],
  ...
  },
  {
    ...
    periodicityOptions: [Periodicity.ANUAL], currentPeriodicity: Periodicity.ANUAL, results: [],
    calcularNotaFinalFn: (v: number | null) => (v !== null && v > 0 ? 1 : 0), 
    valueLabel: "Nº Projetos/Certificações (0, 1, 2+)", valueConsolidationFn: (v: (number | null)[]) => v[0],
    responsibleSector: "Responsável não definido", targetDirection: "none",
  },
  {
  ...
  },
  {
    ...
    periodicityOptions: Object.values(Periodicity), currentPeriodicity: Periodicity.ANUAL, results: [],
    calcularNotaFinalFn: (v: number | null) => (v !== null && v >= 1 ? 0.10 : 0), 
    valueLabel: "Atingiu Meta (1=Sim, 0=Não)", valueConsolidationFn: (v: (number | null)[]) => v[0],
  ...
  },
  {
    ...
    periodicityOptions: Object.values(Periodicity), currentPeriodicity: Periodicity.ANUAL, results: [],
    calcularNotaFinalFn: (v: number | null) => { 
        if (v === null) return 0;
        if (v >= 0.9) return 0.20;
        if (v >= 0.7) return 0.15;
        if (v >= 0.5) return 0.10;
        if (v > 0.3) return 0.05;
        return 0;
    },
    valueLabel: "Frequência (%)", isRate: true, valueConsolidationFn: (v: (number | null)[]) => v[0],
  ...
  },
  {
    ...
    periodicityOptions: [Periodicity.ANUAL], currentPeriodicity: Periodicity.ANUAL, results: [],
    calcularNotaFinalFn: calcularNotaFinalPontuacaoBase, valueLabel: "Realizou Pesquisa (1=Sim, 0=Não)", valueConsolidationFn: (v: (number | null)[]) => v[0],
  ...
  },
  {
    ...
    periodicityOptions: Object.values(Periodicity), currentPeriodicity: Periodicity.ANUAL, results: [],
    calcularNotaFinalFn: (v: number | null) => (v !== null && v <= 0.30 ? 0.10 : 0), 
    valueLabel: "Proporção Diag. Inespecíficos (%)", isRate: true, valueConsolidationFn: (v: (number | null)[]) => v[0],
