...
import { getPeriodLabels, INITIAL_INDICATORS, CURRENT_YEAR as APP_CURRENT_YEAR } from '../constants'; // Using APP_CURRENT_YEAR to avoid conflict
      ...
      <div className="space-y-8">
        ...
        <div className="mt-8 flex flex-col items-center">
          ...
          >
            ...
            {isLoading ? 'Salvando...' : `Salvar Dados Históricos de ${selectedYear}`}
          </button>
          {saveStatus && (
            <div className={`mt-3 p-2.5 rounded-md text-sm flex items-center ${saveStatus.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
              {saveStatus.type === 'success' ? <CheckCircle size={18} className="mr-2"/> : <AlertTriangle size={18} className="mr-2"/>}
              {saveStatus.message}
            </div>
          )}
        </div>
