        ...
        if (!response.ok) {
            ...
            try {
                ...
                if (rawErrorBody.trim() !== '') { // Try to parse only if not empty
                    ...
                    // Body is empty
                    errorMsg = `Erro ${response.status}: ${response.statusText}. O servidor retornou uma resposta de erro vazia. Verifique os logs do servidor Python para detalhes.`;
                ...
                if (rawErrorBody.trim() !== '') {
                    errorMsg = `Erro ${response.status} do backend. Resposta não JSON (início): ${rawErrorBody.substring(0, 200)}... Verifique os logs do servidor Python.`;
                }
                else {
                    // This case should ideally be caught by the empty check above, but as a fallback:
                    errorMsg = `Erro ${response.status}: ${response.statusText}. O servidor retornou uma resposta de erro vazia ou malformada. Verifique os logs do servidor Python.`;
                }
            }
